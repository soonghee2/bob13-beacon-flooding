...
// 맥 주소를 하드코딩할 때 편의를 위해 간단히 만드는 함수
...
int main(int argc, char* argv[]) {
    ...
    uint8_t tagged_params[] = {
        ...
        0x05, 0x04, 0x00, 0x03, 0x00, 0x00,  // TIM
        0x2a, 0x01, 0x04,  // ERP Information
    ...
    while (true) {
        ...
        for (const auto& essid : ssids) {
            ...
            beacon_frame.address2 = make_mac(0x11, 0x11, 0x11, 0x11, 0x11, mac_seed);
            beacon_frame.address3 = make_mac(0x55, 0x55, 0x55, 0x55, 0x55, mac_seed);
            ...
            memcpy(packet + offset, tagged_params, sizeof(tagged_params));
            offset += sizeof(tagged_params);
